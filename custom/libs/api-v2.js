...
function requestBodyLimit(pathValue, contentType, options = {}) {
  ...
  if (path === '/api/v2/problems/import') {
    payloadBytes = Number(options.problemImportArchiveBytes || 100 * 1024 * 1024);
  } else if (/^\/api\/v2\/problems\/\d+\/testdata\/upload$/.test(path)) {
  ...
  } else if (path === '/api/v2/image-host') {
    payloadBytes = Number(options.imageHostBytes || 10 * 1024 * 1024);
  }
...
function ifMatchSatisfied(req, currentEtag, options = {}) {
  const header = req && ((req.get ? req.get('If-Match') : req.headers && req.headers['if-match']) || (req.body && req.body.if_match));
...
const ERROR_CATALOG = Object.freeze(Object.assign(
  ...
  catalogGroup(403, 'You do not have permission to access this resource.', [
    ...
    'SUBMISSION_FORBIDDEN', 'TICKET_FORBIDDEN', 'USER_TAG_FORBIDDEN', 'IMAGE_HOST_FORBIDDEN'
  ]),
  catalogGroup(404, 'The requested resource was not found.', [
    ...
    'VJUDGE_CONNECTION_NOT_FOUND', 'VJUDGE_IMPORT_NOT_FOUND', 'VJUDGE_PROVIDER_NOT_FOUND', 'VJUDGE_REMOTE_PROBLEM_NOT_FOUND', 'AI_AGENT_NOT_FOUND', 'AI_RUN_NOT_FOUND', 'IMAGE_HOST_NOT_FOUND'
  ]),
  ...
  catalogGroup(409, 'The request conflicts with the current resource state.', [
    'AUTHORIZATION_WRITE_FAILED', 'AVATAR_DELETE_FAILED', 'CONTENT_WRITE_FAILED', 'CONTEST_ALREADY_SUBMITTED', 'CONTEST_DELETE_FAILED', 'CONTEST_LOCKED', 'CONTEST_RATING_FINALIZED', 'CONTEST_SUBMIT_FAILED', 'FOLLOW_FAILED',
    ...
    'AI_ACCOUNT_NAME_CONFLICT', 'AI_ACCOUNT_PROVIDER_CONFLICT', 'AI_PROBLEM_SNAPSHOT_MISSING'
  ]),
  catalogGroup(413, 'The submitted content exceeds the allowed size.', [
    'ATTACHMENT_LIMIT_EXCEEDED', 'AVATAR_TOO_LARGE', 'SOURCE_TOO_LARGE', 'AI_SOURCE_TOO_LARGE', 'IMAGE_HOST_TOO_LARGE'
  ]),
  catalogGroup(422, 'One or more fields are invalid.', [
    'AVATAR_UPLOAD_INVALID', 'BULK_IMPORT_INVALID', 'CONTEST_ADMIN_INVALID', 'CONTEST_CONFIG_INVALID', 'CONTEST_PARTICIPANT_IMPORT_INVALID', 'CONTEST_PROBLEM_INVALID',
    'CONTEST_PROBLEM_UNAVAILABLE', 'CONTEST_RANKING_INVALID', 'CONTEST_SCORING_INVALID', 'CONTEST_TEAMS_INVALID', 'USERNAME_INVALID',
    ...
    'RATING_PROFILE_INVALID', 'SUBMISSION_EVENT_INVALID', 'SUBMISSION_STATUS_INVALID', 'AI_PLAN_INVALID', 'AI_LANGUAGE_INVALID', 'AI_SOURCE_INVALID', 'IMAGE_HOST_UPLOAD_INVALID', 'IMAGE_HOST_UNSUPPORTED_TYPE', 'IMAGE_HOST_UNSAFE_CONTENT'
  ]),
  catalogGroup(429, 'Too many requests.', [
    'LOGIN_RATE_LIMITED', 'MFA_CHALLENGE_RATE_LIMITED', 'TICKET_DAILY_LIMIT_REACHED', 'UPSTREAM_RATE_LIMITED', 'AI_RATE_LIMITED'
  ]),
  catalogGroup(500, 'The request could not be completed because of an internal error.', [
    ...
    'PROBLEM_BULK_ARCHIVE_FAILED', 'STANDINGS_REBUILD_FAILED', 'SUBMISSION_LEGACY_WRITE_FAILED', 'TESTDATA_VALIDATION_FAILED', 'USER_UPDATE_FAILED', 'AI_CONTEST_FAILED', 'IMAGE_HOST_UPLOAD_FAILED', 'IMAGE_HOST_DELETE_FAILED'
  ]),
  catalogGroup(502, 'An upstream service returned an invalid response.', [
    'UPSTREAM_AUTH_FAILED', 'UPSTREAM_RESPONSE_INVALID', 'AI_PROVIDER_ERROR', 'AI_RESPONSE_INVALID'
  ]),
  catalogGroup(503, 'A required service is temporarily unavailable.', [
    ...
    'SEARCH_UNAVAILABLE', 'SUBMISSION_DOMAIN_UNAVAILABLE', 'UPSTREAM_UNAVAILABLE', 'API_DOMAIN_DISABLED', 'AI_CREDENTIAL_MISSING', 'IMAGE_HOST_UNAVAILABLE'
  ]),
  catalogGroup(504, 'An upstream service timed out.', [
    'AI_TIMEOUT', 'AI_JUDGE_TIMEOUT'
  ]),
  {
    ...
    AI_CONTEST_FAILED: { status: 500, message: 'The AI contest operation could not be completed.' }
  }
));
