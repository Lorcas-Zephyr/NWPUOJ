...
app.put('/api/v2/admin/help/pages/:slug', async (req, res) => {
  ...
  const currentPage = helpResource(await loadHelpPage()); if (!(req.get('If-Match') || req.body && req.body.if_match)) return api.fail(res, 428, 'PRECONDITION_REQUIRED', 'If-Match is required when updating help content.'); if (!api.ifMatch(req, currentPage)) return api.fail(res, 412, 'ETAG_MISMATCH', 'Help content changed. Reload it before saving.'); const now = Math.floor(Date.now() / 1000);
  try {
